...
async function updateArticleLists(category, searchKeyword) {
  try {
    ...
    //ページ番号を取得    
    // カテゴリーごとのフィルター条件を設定
    const filter = category === 'すべて' ? '' : category === '#all' ? '' : `&filter[field_group_list]=${category.replace("#", "")}`;
    ...
    if (pageNumber == 999) {
      while (currentPage <= totalPages) {
        const response = await (fetch(`${apiUrl}?sort=-field_date,-changed${filter}&page[limit]=${itemsPerPage}&page[offset]=${(currentPage - 1) * itemsPerPage}&filter[title][condition][path]=title&filter[title][condition][operator]=CONTAINS&filter[title][condition][value]=${searchKeyword}`));
    ...
    const response = await fetch(`${apiUrl}?sort=-field_date,-changed${filter}&page[limit]=20&page[offset]=${(pageNumber * 20)}&filter[title][condition][path]=title&filter[title][condition][operator]=CONTAINS&filter[title][condition][value]=${searchKeyword}`);
    const data = await response.json();
